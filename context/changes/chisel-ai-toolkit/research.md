...
- **Read is the only axis that differs** (:150-154): private packages need a long-lived PAT wherever they install (the `preinstall` `GH_PKG_TOKEN` line :158-163 + Cloudflare token-sync :165, :211). **Public packages need no *per-package grant*** — but **CORRECTION (verified post-publish 2026-06-18): they still require *a* token with the `read:packages` scope.** GitHub Packages rejects *all* anonymous npm reads with `401 … authentication token not provided`, even for a package whose visibility is `public`; a token lacking the scope gets `403 … token does not match expected scopes`. So the consumer needs both the `.npmrc` scope mapping **and** a `read:packages` token in their user/CI `.npmrc`. (The earlier "anyone installs with just the scope mapping / public = fully free of read-auth" claim was wrong — public is free of *cost* and of per-package grants, not of the registry's token requirement. The unauthenticated-pull behavior people expect is `ghcr.io` for containers, not `npm.pkg.github.com`.) Confirmed live: with a `read:packages` token + a `package.json` in the consumer dir, `npm install @rafalpuczel/chisel-ai-toolkit` succeeds and installs v1.0.0.
